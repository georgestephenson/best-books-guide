...
- [ ] Drop the `@fastify/static` and `test-exclude` entries from root `overrides` once `@fastify/swagger-ui` widens to `^10` and `@vitest/coverage-v8` moves to `test-exclude` 8 (docs/05 §Supply chain)
