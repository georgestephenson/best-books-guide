...
const stubProbe: HealthProbe = { ping: () => Promise.resolve(true) };

function healthUseCase(): GetHealth {
  return new GetHealth({ clock: stubClock, version: 'test-sha', db: stubProbe, redis: stubProbe });
}
...
describe('unknown route', () => {
  it('returns a 404 problem+json', async () => {
    const app = testServer(healthUseCase());
    await app.ready();
