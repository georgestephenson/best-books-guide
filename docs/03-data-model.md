...
- **Seeds**: the catalogue seed lives in `apps/api/src/seeds/` (dataset + idempotent `applySeed`) with a compiled CLI at `dist/seed-catalogue.js` — it ships to prod for the same reason `migrate.js` does (only `drizzle-orm`, a prod dep). It seeds **public-domain** content (`npm -w apps/api run seed:catalogue`, or `node dist/seed-catalogue.js` on the host), upserting by slug so re-runs converge. It's shippable launch-adjacent content; the editor's real curated picks arrive via the admin UI (M3 slice 5+) and the OL import, never source-controlled.
