...
- **Supply chain**: Dependabot (npm, actions, terraform), `npm audit` in CI (fail on high), CodeQL (free on public repos), third-party GitHub Actions pinned to commit SHAs, `package-lock.json` committed and installs via `npm ci` only (lockfile-exact).
  - The audit gate is unfiltered — no allowlist, no `--omit=dev` — so a high advisory anywhere in the tree blocks the merge. Where a dependency pins a vulnerable transitive version and upstream has no release yet, the fix is a root `overrides` entry (e.g. `@fastify/static` forced past `@fastify/swagger-ui`'s `^9` range for GHSA-83w8-p2f5-377r), verified by the suite rather than waived. Drop each override once its parent widens its range.
